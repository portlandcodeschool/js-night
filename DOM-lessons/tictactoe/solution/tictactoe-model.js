...
var TicTacToeGame = function() { //module returning constructor:
	...
	function Board(endGameFn,gui) { //either arg or both may be null
		...
		function placeMark(x,y,mark) {
			...
			toggleTurn();
			if (gui) {
				gui.mark({x:x,y:y},mark);
				gui.setState(whoseTurn);
			}

			// if endGame callback, check for winner:
			var triple = ((gui || endGameFn) && checkWinner(getMark));
			if (triple || numMarks==9) { //game over...
				if (gui)
					gui.setState(triple);
				if (endGameFn)
					endGameFn(triple);
		...
		this.takeTurn = function(x,y) {
			return placeMark(x,y,whoseTurn);
		}
		...
		if (gui) {
			//gui.setActions({X:this.placeX, O:this.placeO});
			gui.setAction(this.takeTurn);
			gui.setState(whoseTurn);
		}
