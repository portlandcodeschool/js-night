var makeCard = (function () {
    ...
    function Card(id) {// constructor!
        if (!isValid(id,0,this.constructor.highID()))
            delete this.id;
        else
            this.id = id;
    }
        ...
        this.cardID = cardID;
        this.getID = cardID; //alias same method with better name 'getID'
        this.rank = rank;
        this.suit = suit;
        this.color = color;
        this.name = cardName; //NOTE: functions have a built-in property 'name',
...
// but we'll include it for completeness:
    ...
    Card.prototype.rank = function() { // --> 1..13, NaN
        var card = this.id;
        return isValid(card,0,this.constructor.highID()) &&
            Math.floor(card/this.constructor.numSuits())+1;
    };

    Card.prototype.suit = function() { // --> 1..4, NaN
        var card = this.id;
        return isValid(card,0,this.constructor.highID()) &&
            (card%this.constructor.numSuits())+1;
    };
    ...
    if (!isCard(cardB)) return NaN;
        ...
        if (isNaN(diff))
            return NaN;
        return diff==1 || diff==(1-this.constructor.numRanks());
    };
...
// but then we'd need a smarter factory which maintains an index of its instances.
    ...
    Card.prototype.nextInSuit = function() {//--> 0..51,NaN
        var nextCardID = isValid(this.id,0,this.constuctor.highID()) &&
            this.id+this.constructor.numSuits();
        if (nextCardID>this.constructor.highID()) nextCardID-=this.constructor.numIDs;
        return nextCardID;
    };
    
    Card.prototype.prevInSuit = function() {//--> 0..51,NaN
        var prevCardID = isValid(this.id,0,this.constructor.highID()) &&
            this.id-this.constructor.numSuits();
        if (prevCardID<0) prevCardID+=this.constructor.numIDs();
        return prevCardID;
    };
